...
namespace CBGames.Player
{
    ...
    public class SyncPlayer : MonoBehaviourPunCallbacks, IPunObservable
    {
        ...
        #region Ragdoll
        /// <summary>
        /// Used by other MP_ components to know if they should be updating their IK placements or not.
        /// Will not update if currently ragdolled.
        /// </summary>
        /// <returns></returns>
        public virtual bool GetRagdolled()
        {
            return _ragdolled;
        }
...
/*        #region Zipline Addon
        ...
        {
            if (_ragdolled == true && photonView.IsMine)
            {
            ...
            if (photonView.IsMine == false && PhotonNetwork.OfflineMode == false)
            {
                if (sendPosRot == true && !tpc.ragdolled)
                {
                    ...
                    if (distance < 2f)
                    {
                        ...
                        transform.rotation = Quaternion.Lerp(transform.rotation, realRot, Time.deltaTime * _rotationLerpRate);
                    }
                    else
                    {
